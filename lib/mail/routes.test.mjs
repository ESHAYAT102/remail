...
test("builds and recognizes custom collection views", () => {
  ...
  assert.equal(
    mailFolderHref(view, undefined, "gmail/account"),
    "/mail/a/gmail%2Faccount/collection%3ALabel%2FProject%20A",
  );
  assert.deepEqual(
    mailRouteFromPathname(
      "/mail/a/gmail%2Faccount/collection%3ALabel%2FProject%20A",
  ...
  assert.equal(
    mailThreadHref(view, "thread/one", undefined, "gmail/account"),
    "/mail/a/gmail%2Faccount/collection%3ALabel%2FProject%20A/thread/thread%2Fone",
  );
});
...
test("builds stable folder and conversation URLs with list state", () => {
  ...
  assert.equal(
    mailFolderHref("inbox", query, "gmail/account"),
    "/mail/a/gmail%2Faccount/inbox?q=design+review&unread=1&sort=from&order=asc",
  );
  assert.equal(
    mailThreadHref("inbox", "thread/with slash", query, "gmail/account"),
    "/mail/a/gmail%2Faccount/inbox/thread/thread%2Fwith%20slash?q=design+review&unread=1&sort=from&order=asc",
  );
});
...
test("parses folder, conversation, and settings routes", () => {
  ...
  assert.deepEqual(
    mailRouteFromPathname(
      "/mail/a/gmail%2Faccount/inbox/thread/thread%2Fone",
    ),
    {
      kind: "thread",
      accountId: "gmail/account",
